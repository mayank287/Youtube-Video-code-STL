...
int main()
{
    ...
    cin.tie(NULL);
    // Lamda Functions
    auto add = [](int a,int b){ cout << "Sum = " <<  a + b;};
    auto add2 = [](int a,int b){return a + b;};
    add(10,20);
    cout << nline;
    cout << add2(10,20) << nline;

    

    vector<int> v = {0,2,0,6,4};
    // All_of ()
    // all must be true for 1 Output
    cout << all_of(all(v),
    [](int x){return x >= 10;}) << nline;

    // Any_of ()
    // only one must be true for 1 Output
    cout << any_of(all(v),
    [](int x){return x >= 10;}) << nline;

    // none_of ()
    // all must be false for 1 Output
    cout << none_of(all(v),
    [](int x){return x >= 10;}) <<  nline;
